      ...
      <ScrollArea className="flex-1 mb-4 pr-2">
        ...
        >
          {filteredSuppliers.length > 0 ? (
            ...
            // --- MODIFIED ---: Conditional logic for the empty state message
            <div className="text-muted-foreground px-3 py-2">
              {hasSelectedItems ? (
                <>
                  Cart contains items which are from different suppliers. <br />
                  <br /> Remove items or change supplier mappings to create a
                  purchase order.{" "}
                </>
              ) : (
                "Select item(s) to view available suppliers."
              )}
            </div>
          )}
        </RadioGroup>
